 ...
 * 
 * 
 * MAKE WORLD.PLANTVALUE DEINCREMENT EVERYTIME ONE IS EATEN IF ALGAE REPRODUCE IS ON
 * 
 */
...
public class Algae extends AbstOrganism
{
    ...
    //made for slider code
    //static boolean shouldDie = false;
    //code for "talking" between the organism and is plus/minus buttons
    public void text(int number){
        getWorld().showText("" + number, 70, 80 - 20);//shows value, but needs to erase after one print
        getWorld().showText("Algae" , 70, 80 - 40);
        return;
    }
    ...
    public void act() 
    {
        ...
        //    getWorld().addObject(plants.get(plants.size()-1), Greenfoot.getRandomNumber(world.getWidth()), Greenfoot.getRandomNumber(world.getHeight()));
        //}
        text(world.plants.size());
        reproduce();
        feed();
        grow();
        age();

        /*System.out.println(shouldDie);
        if(shouldDie == true){
        die();
        }*/

        //HERE IS THE PROBLEM!!! THIS NEEDS TO BE FIXED!!! --- Moved to slider code
        /*
        if(plants.size() != pSize){
        //*
        //Greenfoot.delay(3);
        ...
        plants.remove(0);//take the first one in the array and kill it
        ...
        //*i/
        //*
        for(int i = plants.size(); i < pSize; i+=0){//runs as many times as # of plants that are needed to be deleted
        //add some new algae
        plants.add(new Algae());
        getWorld().addObject(plants.get(plants.size()-1), Greenfoot.getRandomNumber(getWorld().getWidth()), Greenfoot.getRandomNumber(getWorld().getHeight()));//creates new object
        Greenfoot.delay(1);
        }
        //*i/
        }
        ...
        // System.out.println(plants.size() + "    This is the plants array size second check");
        ...
        //System.out.println("Lifeforms" + plants.size());//Counts
    }
